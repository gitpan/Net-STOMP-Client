...
=item * L<http://stomp.github.com/stomp-specification-1.1.html>

the STOMP 1.1 protocol specification

=item * L<http://fusesource.com/docs/broker/5.4/connectivity_guide/BHIJBDJH.html>
...
Once connected at STOMP level, you can get information about the
broker using the C<server> and C<version> methods. For instance:
...
In fact, the code above only works with STOMP 1.0. In STOMP 1.1, the
C<id> header has been made mandatory so you must use something like:
...
types of frames, except for C<MESSAGE> frames. For these, it is really up to
...
Net::STOMP::Client will extract the C<message-id> for you:
